# MMAP问题导致用户层映射内存缓冲区不一致

问题描述：

内核采用kmalloc分配的内存，其物理地址并没有对齐到页边界，在用户层mmap函数中，导致首地址不一致，而出现了数据错误。

以下是打印信息：

```sh
bsp/drivers/sanway/spi_adc_dm200_hrtimer/sanway_spi_adc_hrtimer.c::esam_mmap(L0466): esam_mmap: mmap_start virt=000000000c2e03c2 phys=0x45000080 pfn_base=0x45000 vma->vm_pgoff=0 vma->vm_start=0x7f9ac70000 size=2097152
```

> [!important]
>
> **由上可见，phys地址为：0x45000080，pfn_base为：0x45000，丢掉了0x80这部分地址，导致用户层映射时出错。**



具体更改参见c代码文件：sanway_spi_adc_hrtimer_no_pwm.c：
