# 关于KSZ8851网卡ping一会就不通的原因解决

### 现象：

从串口终端向PC机ping可以一直ping通，但是从PC机向板卡ping，过一段时间后，就ping不通了，提示：
        
        From 192.168.5.147 icmp_seq=46 Destination Host Unreachable
        From 192.168.5.147 icmp_seq=47 Destination Host Unreachable
        From 192.168.5.147 icmp_seq=48 Destination Host Unreachable

### 解决方法：

经过长时间的分析研究，已经可以得出结论：主要是因为STM32MP1系统处理器的中断不支持电平方式中断，而只支持上升沿或者下降沿中断模式，采用沿模式中断时，需要很小心的处理申请中断和中断回调函数，方式如下：

        1、申请中断必须采用线程方式中断，且中断flag标志必须申请为：IRQF_ONESHOT模式
        ...
        2、中断回调函数中，必须采用while循环，来反复查询是否有中断产生，否则就会丢失中断处理：
        while (!ks->force_quit) {}

        3、可以在数据结构struct ks_net中，增加一个字段，用于控制中断中的循环：
        int			force_quit;
        在open函数中，设置：
        ks->force_quit = 0;
        在stop函数中，设置：
...
这可以作为STM32MP1处理器所有驱动中对于中断处理的一种通用解决方案。
...
完整程序代码如下：
        ...
        struct ks_net {
        	...
        	u32			frame_cnt;
        	int			bus_width;
        	int			force_quit;	//Jason.Wang:添加一个字段，用于中断中的while循环
        ...
        */
        ...
        static int ks8851_probe(struct platform_device *pdev)
        {
        	...
        	if (!io_d) {
        		printk("ks8851 memory resource error!\n");
        	} else {
        		printk("ks8851 start: 0x%08x, iosize: 0x%08x\n", io_d->start, resource_size(io_d));
        	}
        	...
        	if ((id & ~CIDER_REV_MASK) != CIDER_ID) {
        		netdev_err(netdev, "failed to read device ID: 0x%x\n", id);
        		err = -ENODEV;
        		goto err_free;
        	}

        	if (ks_read_selftest(ks)) {
        		netdev_err(netdev, "failed to selftest.\n");
        		err = -ENODEV;
        		goto err_free;
        	}
